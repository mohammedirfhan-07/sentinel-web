...
if not GROQ_API_KEY:
    ...
    print("2. Add your Groq API key (from groq.com)")
    print("=" * 60)
    sys.exit(1)
...
def analyze_with_groq(content: str) -> dict:
    """Send content to Groq for manipulation analysis."""
    try:
        ...
        if response.status_code == 401:
            raise HTTPException(status_code=401, detail="Authentication failed: Invalid Groq API Key. Ensure your real key is in the .env file.")
...
def scrape_url(url: str) -> dict:
    ...
    try:
        ...
        if domain in ["twitter.com", "www.twitter.com", "x.com", "www.x.com"]:
            api_url = parsed._replace(netloc="api.vxtwitter.com").geturl()
            try:
                r = requests.get(api_url, timeout=10)
                if r.status_code == 200:
                    data = r.json()
                    tweet_text = data.get("text", "")
                    author = data.get("user_name", "Twitter User")
                    if tweet_text:
                        return {
                            "success": True,
                            "content": f"Tweet by {author}:\n{tweet_text}",
                            "metadata": {
